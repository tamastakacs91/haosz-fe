<template>
  <div>
    <v-container>
      <v-row>
        <v-col class="text-center text-h5 text-md-h4 mt-5"
          >Regisztráció Orvosoknak</v-col
        >
      </v-row>
      <v-col cols="12" md="8" offset="md-2">
        <v-card outlined ref="form">
          <v-col cols="12" md="10" offset="md-1">
            <v-text-field
              label="NÉV"
              ref="name"
              v-model="doctor.data.name"
              filled
              class="mt-4"
              :rules="[validationRules.required]"
              validate-on-blur
            ></v-text-field>
            <v-text-field
              label="ORVOSI PECSÉTSZÁM"
              ref="sealNumber"
              v-model="doctor.data.sealNumber"
              filled
              :rules="[validationRules.required]"
              validate-on-blur
            ></v-text-field>
            <v-text-field
              label="MUNKAHELY"
              ref="workPlace"
              filled
            ></v-text-field>
            <div class="d-flex flex-wrap">
              <v-text-field
                label="MOBIL"
                ref="mobile"
                v-model="doctor.data.mobile"
                filled
                class="mr-md-5"
                hint="Formátum: +36 20/30/70 xxxxxxx"
                :rules="[validationRules.required, validationRules.mobile]"
                validate-on-blur
              ></v-text-field>
              <v-text-field
                label="E-MAIL"
                ref="email"
                v-model="doctor.data.email"
                filled
                type="email"
                :rules="[validationRules.required]"
                validate-on-blur
              ></v-text-field>
            </div>
            <div class="mt-4 mb-4">Számlázási Adatok:</div>
            <v-text-field label="CÉG NEVE" filled></v-text-field>
            <v-text-field label="CÍM" filled></v-text-field>
            <v-text-field label="ADÓSZÁM" filled></v-text-field>
            <v-text-field label="KAPCSOLATTARTÓ NEVE" filled></v-text-field>
            <div class="d-flex flex-wrap">
              <v-text-field
                label="MOBIL"
                filled
                class="mr-md-5"
                hint="Formátum: +36 20/30/70 xxxxxxx"
              ></v-text-field>
              <v-text-field label="E-MAIL" filled type="email"></v-text-field>
            </div>
            <div class="mt-4 mb-4">Jelszó:</div>
            <v-text-field
              label="JELSZÓ"
              ref="password"
              v-model="doctor.password.password"
              filled
              :hint="'Legalább 8 karakter, tartalmaznia kell kisbetűt, nagybetűt, számot és egy különleges karaktert.'"
              :rules="[validationRules.required, validationRules.password]"
              validate-on-blur
              :append-icon="show1 ? 'mdi-eye' : 'mdi-eye-off'"
              :type="show1 ? 'text' : 'password'"
              @click:append="show1 = !show1"
              @input="$emit('validatePassword', $event)"
            ></v-text-field>
            <div class="mb-4">Jelszó ismét:</div>
            <v-text-field
              label="JELSZÓ ISMÉT"
              ref="passwordAgain"
              v-model="doctor.password.passwordAgain"
              filled
              :rules="[validationRules.required, validationRules.passwordAgain]"
              validate-on-blur
              :append-icon="show2 ? 'mdi-eye' : 'mdi-eye-off'"
              :type="show2 ? 'text' : 'password'"
              @click:append="show2 = !show2"
            ></v-text-field>
          </v-col>
        </v-card>
      </v-col>
      <v-row>
        <v-col cols="12" md="8" offset="md-2" class="text-center">
          <v-btn color="primary" width="50%" @click="submit"
            >Regisztráció</v-btn
          >
        </v-col>
      </v-row>
      <slot name="terms"></slot>
    </v-container>
    <slot name="more-info"></slot>
  </div>
  <!-- <div>
    <v-container>
      <v-row>
        <v-col class="text-center text-h5 text-md-h4 mt-5">Regisztráció Orvosoknak</v-col>
      </v-row>
      <v-col cols="12" md="8" offset="md-2">
        <v-card outlined ref="form">
          <v-col cols="12" md="10" offset="md-1">
            <slot name="fields"></slot>
          </v-col>
        </v-card>
      </v-col>
      <v-row>
        <v-col cols="12" md="8" offset="md-2" class="text-center">
          <v-btn color="primary" width="50%" @click="$emit('click')">Regisztráció</v-btn>
        </v-col>
      </v-row>
    </v-container>
  </div>-->
</template>

<script>
export default {
  props: ['validationRules'],
  data() {
    return {
      doctor: {
        data: {
          name: '',
          sealNumber: '',
          workPlace: '',
          mobile: '',
          email: '',
        },
        billing: {
          name: '',
          address: '',
          taxNumber: '',
          contact: '',
          mobile: '',
          email: '',
        },
        password: {
          password: '',
          passwordAgain: '',
        },
      },
      show1: false,
      show2: false,
      errors: false,
    };
  },
  methods: {
    checkFieldValiditiy(data) {
      Object.keys(data).forEach((field) => {
        if (this.$refs[field].validate() === false) this.errors = true;
      });
    },
    submit() {
      this.errors = false;
      this.checkFieldValiditiy(this.doctor.data);
      this.checkFieldValiditiy(this.doctor.password);

      if (this.errors === true) return console.log('Hibás kitöltés!');
      console.log(this.doctor.data);
    },
  },
};
</script>
