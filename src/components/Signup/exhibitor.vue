        ...
        <v-card outlined ref="form">
          <v-col cols="12" md="10" offset="md-1">
            ...
            <v-text-field
              ...
              :rules="[rules.required]"
              validate-on-blur
              @input="$emit('input', $event, 'data', 'address')"
            ></v-text-field>
            <v-text-field
              label="ADÓSZÁMA"
              ref="taxNumber"
              ...
              :rules="[rules.required]"
              validate-on-blur
              @input="$emit('input', $event, 'data', 'taxNumber')"
            ></v-text-field>
            <v-text-field
              ...
              :rules="[rules.required]"
              validate-on-blur
              @input="$emit('input', $event, 'data', 'contact')"
            ></v-text-field>
      ...
      <v-row>
        <v-col cols="12" md="8" offset="md-2" class="text-center">
          <v-btn
            color="primary"
            width="50%"
            @click="validate(exhibitor.data, exhibitor.password)"
            >Regisztráció</v-btn
          >
        </v-col>
      </v-row>
